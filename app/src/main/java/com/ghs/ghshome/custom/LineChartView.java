...
import com.ghs.ghshome.bean.PointToDraw;
...
public class LineChartView extends View {
    ...
    private List<Integer> texts_X = new ArrayList<>();//x轴上的文字

    private PointToDraw[] linePoints;
    ...
    private float rulerValue = rulerValueDefault;//刻度单位跨度
    private int rulerValuePadding;//刻度单位与轴的间距
    private int rulerValuePaddingDP = 8;//刻度单位与轴的间距默认dp
    private float heightPercent = 0.618f;

    private int lineColor = Color.parseColor("#FFFF7C3B");//曲线颜色
    private float lineWidthDP = 1f;//曲线宽度dp

    private int pointColor = Color.parseColor("#FFFF7C3B");//锚点颜色
    private float pointWidthDefault = 6f;
    private float pointWidthDP = pointWidthDefault;//锚点宽度dp

    private int tableColor = Color.parseColor("#FF000000");//表格线颜色
    ...
    private void resetParam() {
        ...
        linePoints = new PointToDraw[dataList.size()];
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        int width = tablePadding + getTableEndInt() + getPaddingLeft() + getPaddingRight();//计算自己的宽度
    ...
    private float getTableStart() {
        return isShowTable ? stepStart + tablePadding : stepStart;
    }

    private int getTableEndInt() {
        return Math.round(isShowTable ? stepEnd + tablePadding : stepEnd);
    }
    private float getTableEndFloat() {
    ...
    private void drawTable(Canvas canvas) {
        ...
        float tableEnd = getTableEndFloat();

        float rulerCount = maxValue / rulerValue;
        float rulerMaxCount = maxValue % rulerValue > 0 ? rulerCount + 1 : rulerCount;
        float rulerMax = rulerValue * rulerMaxCount + rulerValueDefault;
...
//        tablePath.lineTo(tableEnd, 0);//标尺x轴
        ...
        float startValue = minValue - (minValue > 0 ? 0 : minValue % rulerValue);
        float endValue = (maxValue + rulerValue);

        //标尺y轴连接线
        do {
            tablePath.reset();
            float startHeight = -getValueHeight(startValue);
            ...
            if (rulerValue==startValue) {
                tablePaint.setPathEffect(new DashPathEffect(new float[]{8, 8}, 0));
                canvas.drawPath(tablePath, tablePaint);
            }else{
                tablePaint.setPathEffect(new DashPathEffect(new float[]{8, 0}, 0));
                canvas.drawPath(tablePath, tablePaint);
            }
    ...
    private void drawRulerXValue(Canvas canvas) {
        ...
        for (int i = 0; i < linePoints.length; i++) {
            PointToDraw point = linePoints[i];
            int text = texts_X.get(i);
            if (point == null) break;
            drawRulerXText(canvas, String.valueOf(text), linePoints[i].x, -10);
        }
    }
     ...
     * @return
     */
    private float getValueHeight(float value) {
        float valuePercent = (Math.abs(value - minValue) * 100f / (Math.abs(maxValue - minValue) * 100f));//计算value所占百分比
        return  (getViewDrawHeight() * valuePercent + bottomSpace + 0.5f);//底部加上间隔
    }
    ...
    private void setupLine() {
        ...
        float stepTemp = getTableStart();
        PointToDraw pre = new PointToDraw();
        pre.setX(stepTemp);
        pre.setY( -getValueHeight(dataList.get(0).getValue()));//坐标系从0,0默认在第四象限绘制
        linePoints[0] = pre;
        ...
        for (int i = 1; i < dataList.size(); i++) {
            ...
            PointToDraw next = new PointToDraw();
            next.setX(stepTemp += stepSpace);
            next.setY( -getValueHeight(data.getValue()));

            if (isBezierLine) {
                float cW = pre.x + stepSpace / 2;

                PointToDraw p1 = new PointToDraw();//控制点1
                p1.setX(cW);
                p1.setY( pre.y);

                PointToDraw p2 = new PointToDraw();//控制点2
                p2.setX(cW);
                p2.setY( next.y);
     ...
     * 设置x轴上的数据
     */
    public void setXTexts(List<Integer> integers) {
        if (integers!=null&&!integers.isEmpty()) {
            texts_X.clear();
            texts_X.addAll(integers);
     ...
     * @param dataList
     */
    public void setData(   List<LineChartView.Data> dataList) {
        if (dataList == null) {
...
//            }
//        }).getValue();
        maxValue = getMax_Max( (Float[]) datas.toArray(new Float[datas.size()]));
        minValue = getMax_Min( (Float[]) datas.toArray(new Float[datas.size()]));
     ...
     * @return
     */
    public static Float getMax_Max(Float[] arr){
        float max = arr[0];
        float min = arr[0];
     ...
     * @return
     */
    public static Float getMax_Min(Float[] arr){
        float max = arr[0];
        float min = arr[0];
        for(int x = 1;x<arr.length;x++){
            if(max<arr[x])
     ...
     * @param space
     */
    public void setRulerYSpace(float space) {
        if (space <= 0) {
