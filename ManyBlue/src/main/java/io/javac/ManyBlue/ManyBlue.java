...
public class ManyBlue {
    ...
    public static void blueStartService(Context context) {
        if (!runing(context) && blueSupport(context)) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                context.startForegroundService(new Intent(context, BlueLibraryService.class));
            } else {
    ...
    public static boolean runing(Context context) {
        ...
        if (myAM.getRunningServices(Integer.MAX_VALUE) != null) {
...
//    }
     ...
     * @return
     */
    public static BlueGattCallBack getDeviceGatt(Object tag) {
        BlueGattCallBack gatt = null;
//        if ((gatt = BluetoothGattManager.getGatt(tag)) != null)
//            return gatt.getGatt();
        return BluetoothGattManager.getGatt(tag);
    }
